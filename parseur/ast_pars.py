        ...
        if (not est_terminal(sommet_pile)):
            ...
            if rule is not None:
                if rule != ["epsilon"]:
                    ...
                    regleC = rule.copy()
                    pile_arbre.append(
                        [sommet_pile, regleC])  # on empile le sommet de la pile et la règle correspondante
                ...
                print(
                    "Erreur : la table ne contient pas de règle pour le couple (sommet_pile,token_lu). Sommet de la pile : ",
                    sommet_pile, " valeur Token lu : ", lexical_table[token_lu[0]][token_lu[1]], " Ligne : ",
                    token_lu[2])
        ...
        # Cas 2 : si le sommet de la pile est un terminal (<type_token, valeur_token> donc forme ou "eof")
            ...
            if (sommet_pile == (0, 32)):  # si le sommet de la pile est eof
                if (token_lu[1] == 32) and (token_lu[0] == 0):
                    ...
                    erreur = True
                    print("Erreur : la pile est vide mais la liste de token n'est pas finie. Sommet de la pile : ",
                          sommet_pile, " valeur Token lu : ", lexical_table[token_lu[0]][token_lu[1]], " Ligne : ",
                          token_lu[2])
